...
// void allocate_lazy(uint addr) {
//     // Implementation for lazy allocation
// }

// void allocate_locality(uint addr) {
//     // Implementation for locality-aware allocation
// }

// // You could also declare the function pointer here, if it's only used for page fault handling
// void (*allocate_page)(uint) = allocate_lazy; // Default allocation strategy
...
trap(struct trapframe *tf)
{
  ...
  switch(tf->trapno){
  case T_PGFLT:
  // Page fault handler enhancement for locality consideration
char *allocatedPage;
...
for(int pageIndex = 0; pageIndex < 3; pageIndex++) { // Extend over three pages
    allocatedPage = kalloc(); // Secure a new page
    if(allocatedPage == 0){
